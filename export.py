...
def ensure_master_csv_schema(df: pd.DataFrame, log_fn=None) -> tuple[pd.DataFrame, bool, bool]:
    ...
    # ONLY an in-memory read-representation fix (an empty cell and "" round-trip identically on
    # disk), so it must NOT count as a schema modification — otherwise importing a freshly
    # saved master would needlessly raise the "Master CSV Updated" dialog. Logged at DEBUG.
    if 'Info_Sheet' in df.columns:
        ...
        if not _info_norm.equals(df['Info_Sheet']):
            ...
            logger.debug("Normalized 'Info_Sheet' to string dtype (in-memory read fix; "
                         "not a schema change).")
